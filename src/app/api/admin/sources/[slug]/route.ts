...
export async function PATCH(req: Request, { params }: { params: { slug: string } }) {
  ...
  if ('config' in body) {
    ...
    if (targetAdapter === 'greenhouse_api' || targetAdapter === 'workable_api' || targetAdapter === 'lever_api') {
      if (!cfg || typeof cfg.boardSlug !== 'string' || !cfg.boardSlug.trim()) {
        return NextResponse.json({ error: `${targetAdapter} nécessite config.boardSlug` }, { status: 400 })
      }
    }
