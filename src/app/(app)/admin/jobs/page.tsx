...
export default function AdminJobsPage() {
  ...
  async function handleDelete(jobId: string) {
    if (!confirm(t.adminJobs.confirmDelete)) return
  ...
  async function handleReactivate(jobId: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(t.adminJobs.reactivateError)
      }
    } catch (err) {
      console.error('Error:', err)
      alert(t.adminJobs.reactivateError)
  ...
  const categoryLabels = t.categories as { [key: string]: string }
  ...
  return (
    <div className="px-4 sm:px-5 lg:px-7 py-5 pb-24 lg:pb-10 max-w-5xl">
      <h1 className="text-xl sm:text-2xl font-extrabold text-stone-900 mb-1">{t.adminJobs.title}</h1>
      <p className="text-sm text-stone-500 mb-6">{t.adminJobs.subtitle}</p>

      {/* Stats */}
      <div className="grid grid-cols-2 gap-3 sm:gap-4 mb-6">
        <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-lg p-4 border border-green-200">
          <div className="text-2xl font-bold text-green-700">{activeCount}</div>
          <div className="text-xs font-medium text-green-600">{t.adminJobs.activeJobs}</div>
        </div>
        <div className="bg-gradient-to-br from-stone-50 to-stone-100 rounded-lg p-4 border border-stone-200">
          ...
          <div className="text-xs font-medium text-stone-600">{t.adminJobs.inactiveJobs}</div>
        </div>
      </div>
      ...
      {/* Jobs Table */}
      {loading ? (
        <div className="text-center py-12 text-stone-500">{t.adminJobs.jobsLoading}</div>
      ) : filteredJobs.length === 0 ? (
        <div className="text-center py-12 text-stone-500">{t.adminJobs.noJobs}</div>
      ) : (
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b-2 border-stone-200">
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.titleCol}</th>
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.companyCol}</th>
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.stateCol}</th>
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.categoryCol}</th>
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.dateCol}</th>
                <th className="text-left px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.statusCol}</th>
                <th className="text-center px-4 py-3 font-semibold text-xs sm:text-sm text-stone-700">{t.adminJobs.actionsCol}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={job.id} className="border-b border-stone-100 hover:bg-stone-50 transition">
                  ...
                  <td className="px-4 py-3 text-xs sm:text-sm">
                    <span className={`inline-block px-2.5 py-1 rounded-full text-xs font-semibold ${
                      ...
                      {job.active ? t.adminJobs.activeStatus : t.adminJobs.inactiveStatus}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex gap-1 sm:gap-2 justify-center flex-wrap">
                      ...
                      {job.active && (
                        ...
                        >
                          {deletingId === job.id ? '...' : t.common.delete}
                        </button>
                      )}
                      {!job.active && (
                        <button
                        ...
                        >
                          {reactivatingId === job.id ? '...' : t.adminJobs.reactivate}
                        </button>
                      )}
                    </div>
                  </td>
